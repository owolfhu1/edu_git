...
const gitCommand = async (args, context) => {
  ...
  if (subcommand === 'stash') {
    ...
    if (action === 'apply' || action === 'pop') {
      ...
      for (const file of entry.files) {
        ...
        try {
          if (file.isBinary) {
            currentBytes = await pfs.readFile(absPath)
          } else {
        ...
        if (file.isBinary) {
          const stashedBinary = file.stashedBinary ?? null
          const baseBinary = file.baseBinary ? decodeBinary(file.baseBinary) : null
          if (stashedBinary === null) {
            if (!hasCurrent) {
              continue
            }
            if (baseBinary && currentBytes && !buffersEqual(currentBytes, baseBinary)) {
          ...
          if (file.wasUntracked && !hasCurrent) {
            await pfs.writeFile(absPath, decodeBinary(stashedBinary))
            continue
          }
          if (baseBinary && currentBytes && !buffersEqual(currentBytes, baseBinary)) {
            conflictFiles.push(file.path)
    ...
    if (action === 'push' || action === 'save') {
      ...
      for (const [filepath, head, workdir, stage] of statusMatrix) {
        ...
        let stashedBinary = null
        let isBinary = false
        try {
          const rawContent = await pfs.readFile(absPath)
          isBinary = isBinaryContent(rawContent)
          if (isBinary) {
            stashedBinary = encodeBinary(rawContent)
            stashedText = null
          } else {
            stashedText = new TextDecoder().decode(rawContent)
          }
        } catch (error) {
          stashedText = null
        }
        ...
        let baseBinary = null
        if (isBinary) {
          const baseBlob = await readBlobAtPathRaw(fs, root, gitdir, 'HEAD', filepath)
          baseBinary = baseBlob ? encodeBinary(baseBlob) : null
        } else {
          baseText = await readBlobAtPath(fs, root, gitdir, 'HEAD', filepath)
        }
        files.push({
          path: filepath,
          baseText: baseText ?? '',
  ...
  if (subcommand === 'merge') {
    ...
    try {
      ...
      await refreshTree()
      await Promise.all([
        pfs.unlink(`${gitdir}/MERGE_HEAD`).catch(() => {}),
        pfs.unlink(`${gitdir}/MERGE_MSG`).catch(() => {}),
      ])
      if (result?.alreadyMerged) {
        appendOutput(['Already up to date.'])
        return
  ...
  if (subcommand === 'cherry-pick') {
    try {
      ...
      const { root, gitdir } = repo
      const cherryArgs = args.slice(1)
      const mainlineFlagIndex = cherryArgs.findIndex(
        (arg) => arg === '-m' || arg === '--mainline'
      )
      let mainline = null
      if (mainlineFlagIndex !== -1) {
        const mainlineRaw = cherryArgs[mainlineFlagIndex + 1]
        if (!mainlineRaw) {
          appendOutput(['fatal: cherry-pick -m requires a mainline parent'])
          return
        }
        const parsedMainline = Number(mainlineRaw)
        if (!Number.isInteger(parsedMainline) || parsedMainline < 1) {
          appendOutput(['fatal: mainline must be a positive integer'])
          return
        }
        mainline = parsedMainline
        cherryArgs.splice(mainlineFlagIndex, 2)
      }
      const action = cherryArgs[0]
      const currentBranch = await git.currentBranch({
        fs,
        dir: root,
        gitdir,
      ...
      const parentCount = commit.parent?.length || 0
      if (mainline && parentCount < 2) {
        appendOutput(['fatal: -m is only valid for merge commits'])
        return
      }
      if (!mainline && parentCount > 1) {
        appendOutput(['fatal: cherry-pick of a merge commit requires -m'])
        return
      }
      if (mainline && mainline > parentCount) {
        appendOutput([
          `fatal: mainline must be between 1 and ${parentCount}`,
        ])
        return
      }
      const headOid = await git.resolveRef({
      ...
      const { conflictFiles, changedFiles } = await applyCommitChanges({
        ...
        commitOid,
        headOid,
        parentIndex: mainline ? mainline - 1 : 0,
      })
      await refreshTree()
      if (conflictFiles.length > 0) {
